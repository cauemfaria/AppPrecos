...
import { ProductSearchItem, Market, CompareResponse } from '../types';
import { 
  Search, Plus, Minus, Trash2, Store, 
  ArrowRightLeft, Check, X, ChevronRight,
...
const ShoppingListPage: React.FC = () => {
  ...
  const handleCompare = async () => {
    if (shoppingList.length === 0 || selectedMarketIds.length === 0) return;
  ...
  return (
    <div className="p-6 pb-32">
      ...
      {/* Market Selector Bar */}
      <div className="fixed bottom-24 md:bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-lg bg-white p-4 rounded-3xl shadow-2xl border border-gray-100 z-40">
        <div className="flex items-center justify-between gap-4">
          ...
          >
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Markets</p>
              <p className="font-bold text-gray-800 truncate">
                {selectedMarketIds.length === 0 
                  ? 'None selected' 
                  : `${selectedMarketIds.length} markets selected`}
              </p>
            </div>
          </button>
          ...
          <button 
            ...
            className="bg-blue-600 disabled:bg-gray-200 text-white p-3 px-6 rounded-2xl font-bold flex items-center gap-2 hover:bg-blue-700 transition-all active:scale-95 disabled:active:scale-100"
          >
            {isComparing ? (
                  ...
                  {searchResults.map((product, idx) => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400">Found in {product.markets_count} markets • R$ {product.min_price.toFixed(2)} - R$ {product.max_price.toFixed(2)}</p>
                      </div>
              ...
              {markets.map((market) => {
                ...
                return (
                  <button 
                    ...
                    onClick={() => toggleMarketSelection(market.market_id)}
            ...
            <div className="flex-1 overflow-auto p-4 md:p-6">
                ...
                <div className="overflow-hidden border border-gray-100 rounded-3xl">
                  <table className="min-w-full divide-y divide-gray-100">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider sticky left-0 bg-gray-50 z-10">Product</th>
                        {Object.entries(comparisonResult.markets).map(([id, name]) => (
                          <th key={id} className="px-6 py-4 text-center text-xs font-bold text-gray-400 uppercase tracking-wider">{name}</th>
                        ))}
                      </tr>
                    </thead>
                      ...
                      {comparisonResult.comparison.map((row, idx) => (
                        <tr key={idx} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 sticky left-0 bg-white group-hover:bg-gray-50 z-10 border-r border-gray-50">
                            <p className="text-sm font-bold text-gray-800 min-w-[150px]">{row.product_name}</p>
                            <p className="text-[10px] text-gray-400">EAN: {row.ean}</p>
                          </td>
                                ...
                                {price === null ? (
                                  ...
                                  <div className="flex flex-col items-center">
                                    <span className={`text-sm font-bold px-3 py-1 rounded-lg ${
                                      isMin ? 'bg-green-100 text-green-700 border border-green-200' :
                                      isMax ? 'bg-red-50 text-red-700 border border-red-100' :
                                      'text-gray-700'
                                    }`}>
                                      R$ {price.toFixed(2)}
                                    </span>
                                    {isMin && <span className="text-[8px] font-bold text-green-600 uppercase mt-1">Best Price</span>}
                                  </div>
                                )}
                              </td>
                            );
              ...
              <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 flex items-start gap-4">
                  <div className="p-3 bg-blue-600 rounded-2xl text-white"><TrendingDown className="w-6 h-6" /></div>
                  <div>
                    <h4 className="font-bold text-blue-900 mb-1">Price Tip</h4>
                    <p className="text-sm text-blue-700 leading-relaxed">Green prices indicate the lowest available price for that item across your selected markets.</p>
                  </div>
                </div>
                <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 flex items-start gap-4">
                  <div className="p-3 bg-gray-400 rounded-2xl text-white"><AlertCircle className="w-6 h-6" /></div>
                  <div>
                    <h4 className="font-bold text-gray-900 mb-1">N/A Items</h4>
                    <p className="text-sm text-gray-500 leading-relaxed">"N/A" means the product was not found in that specific market's database.</p>
                  </div>
                </div>
              </div>
            </div>
